...
/**
 * cleat the canvas and add a border
 *
 * @param fillStyle
 * @param borderOptions
 * @returns {THREEx.DynamicTexture}
 */
THREEx.DynamicTexture.prototype.clearWithBorder = function (fillStyle, strokeStyle, lineWidth) {
    // depends on fillStyle
    if (fillStyle !== undefined) {
        this.context.fillStyle = fillStyle
        this.context.fillRect(0, 0, this.canvas.width, this.canvas.height);
    ...
    this.context.lineWidth = lineWidth;//边框宽度
    this.context.strokeStyle = strokeStyle;//边框颜色
...
THREEx.DynamicTexture.prototype.drawTextCooked = function (text, options) {
    ...
    var params = {
        ...
        isRect:options.isRect!== undefined ? options.isRect:0,
    }
    context.save()
    ...
    while (text.length > 0) {
        ...
        if(params.isRect) {
            ...
            this.context.strokeRect(5, 5, canvas.width - 10, canvas.height - 10);
        }
